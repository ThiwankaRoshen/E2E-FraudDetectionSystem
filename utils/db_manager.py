...
class DatabaseManager:
    ...
    def _create_sqlite_tables(self):
        ...
        # Table 1: fraud_predictions - stores individual prediction results
        cursor.execute("""
        ...
        # Table 2: high_risk_users - tracks users with high fraud probability
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS high_risk_users (
                ...
                UNIQUE(user_id, detected_at)
            )
        """)
        
        # Table 3: fraud_metrics_hourly - hourly aggregated fraud statistics
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS fraud_metrics_hourly (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                unique_browsers INTEGER
            )
        """)
        
        # Table 4: fraud_metrics_daily - daily aggregated fraud statistics
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS fraud_metrics_daily (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # Table 5: source_analysis - fraud analysis by traffic source
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS source_analysis (
                ...
                avg_risk_score REAL,
                avg_purchase_value REAL,
                UNIQUE(analysis_date, source)
            )
        """)
        
        # Table 6: browser_analysis - fraud analysis by browser type
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS browser_analysis (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                analysis_date DATE NOT NULL,
